#!/usr/bin/env python3
"""Fetch lyrics via the Genius API using songlist_master.csv."""
...
def parse_song_list() -> Dict[str, Dict[str, str]]:
    if not SONGLIST_FILE.exists():
        raise SystemExit(f"Song list CSV missing at {SONGLIST_FILE}")
    songs: Dict[str, Dict[str, str]] = {}
    with SONGLIST_FILE.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            ...
            if not title:
                continue
            songs[title] = {"artist": artist, "raw": row}
    return songs
...
def determine_targets(options: argparse.Namespace, songs: Dict[str, Dict[str, str]]) -> List[str]:
    if not options.song:
        raise SystemExit("Use --song (repeatable) to specify which songs to fetch.")
    targets: List[str] = []
    for title in options.song:
        if title not in songs:
            raise SystemExit(f"Song '{title}' was not found in the CSV list.")
        destination = LYRICS_DIR / f"{title}.txt"
        if destination.exists() and not options.overwrite:
            print(f"Skipping '{title}' (lyrics already exist; use --overwrite to refresh).")
            continue
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Download lyrics via Genius API into song_data/lyrics based on songlist_master.csv."
    )
    parser.add_argument(
        "--song",
        action="append",
        help="Exact song title from songlist_master.csv (repeat to fetch multiple titles).",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
